...
- **Before quality even enters the picture:** most packs (illustrative: ~7 of 10) ship as big **atlas sheets** — one PNG holding hundreds of fused tiles. Cutting, naming, and autotile combinatorics land on *you*. [§4.4](#44-the-delivery-format-audit--gap-0-atomization)
...
|---|---|---:|---:|---:|---:|---:|
| A–G | atlas-only | 0 | 2–6 each | mostly | some | 120–480 each |
| H | mixed | 24 | 2 | 2 | 0 | 210 |
| I–J | atlas-only | 0 | 3–4 | all | 0 | 260–400 |

*(Placeholder table — generated by `scripts/audit-delivery-format.ts` on the real packs; the tool's output pastes over this in the exact shape.)*

**Pooled (illustrative):** ~70 % of packs ship atlas-only · ~85 % of all delivered tiles arrive *inside* sheets · most sheets are grid-aligned (machine-cuttable with the right tooling), a minority tightly packed/irregular (manual work).
...
0. **Atomize before anything:** if the pack ships as atlas sheets, establish the grid (or accept manual cutting) first — the quality of an uncut sheet is unmeasurable, and so is your gameplay.
