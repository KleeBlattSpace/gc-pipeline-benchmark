...
### The expectation gap, measured: 240 tiles from the most-downloaded free tilemap packs on itch.io — what expert-made assets *demand from users* before gameplay can happen. (No, the packs aren't the problem. Readiness ≠ quality — a "Reject" tile can be beautiful art that simply isn't engine-ready yet. And this time the instrument itself is on trial: half the study is about where naive QC fails *good* art.)

> **DATA STATUS: STRUCTURAL — v1.0.**
> Real packs, real inventory, real audit, real 240-tile pilot scores — but scored with the **public pixel baseline** (`pixel-baseline@1`), a transparent heuristic, *not* the pinned TileFix core. The headline finding is methodological: standalone cell heuristics **do not transfer** to assembly-designed tiles, so the gate percentages below measure the *instrument's limits* as much as the packs. They are reported stratified and forensically, never as a readiness verdict on any pack.
...
- **Gap 0 is real and huge:** 45 tile-bearing sheets hold **14,182 addressable cells**; exactly one pack ships pre-cut tiles (101 loose isometric blocks). **99.3 % of tile content arrives locked inside sheets** — cutting, naming, and autotile combinatorics land on *you*. [§4.4](#44-the-delivery-format-audit--gap-0-atomization)
...
Download counts measure *popularity on the store page*. Engines measure *pixels at runtime*. Nothing in between measures **technical readiness for tiling** — that is the gap this study (and the benchmark behind it) addresses: a reproducible definition of "production-ready" that you can run in your own CI, for free, on every pull request.
...
Concrete vignettes — the study's core. All scores real (pixel baseline); provenance (sheet + cell) in `data/registry.json`; forensic renders were produced locally and verified by eye (not committed — aggregates only).
...
0. **Atomize before anything:** if the pack ships as atlas sheets, establish the grid (ship a `.tsx`/grid note like pack G — it was the only pack where the grid needed zero reverse-engineering) and cut before judging anything. The quality of an uncut sheet is unmeasurable, and so is your gameplay.
...
But notice the shape of the problem this pilot mapped: scoring tells you a brick wall is "Reject 44" while its assembly is seamless; pixel touch-ups improve averages while dropping individuals a gate; finding the grid defeats naive detection on most real sheets. Standalone, context-free operations keep hitting the same wall — **assembly context**. That transformation — neighbor-aware scoring, degradation-penalized fixing, set-level mapping — is a different machine from a cell grader. In the TileSmith pipeline that machine is the **TileFix Doctor** (Upload → Doctor → Set → Map → Export), and it runs locally in your browser — no account needed to start. When your CI says Review or Reject, the Doctor is the piece that answers the question this study deliberately leaves open: *broken, or just unassembled?*
